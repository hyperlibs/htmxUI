...
test.describe('HTMXUI Live Demos & HyperAdmin Full Verification Suite', () => {
  ...
  test('2. HyperAdmin Full Recreation (/demo/tailadmin) renders all dashboards, apps, forms, tables, and settings', async ({ page }) => {
    await page.goto(`${BASE_URL}/demo/tailadmin`);
    await expect(page).toHaveTitle(/HyperAdmin Dashboard/);
    ...
    await expect(page.locator('text=Sign in to HyperAdmin')).toBeVisible();
  });
